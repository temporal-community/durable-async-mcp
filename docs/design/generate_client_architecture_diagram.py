...
E.append(
    T(
        ...
        300,
        16,
        "task_id: invoice-abc  ·  state: polling",
        size=11,
        color="#495057",
        align="left",
    )
...
E.append(
    A("ap_up1", 399, 482, 650, 188, bidir=True, dashed=True, color="#2f9e44", sw=1)
)
E.append(
    A("ap_up2", 659, 482, 660, 188, bidir=True, dashed=True, color="#2f9e44", sw=1)
)

# --- MCP Server ---
E.append(R("ms_out", 1150, 48, 390, 300, bg="#e7f5ff", stroke="#228be6", sw=3))
E.append(
    T("ms_lbl", 1162, 52, 360, 26, "MCP Server", size=15, color="#228be6", align="left")
)
E.append(
    T(
        "ms_sub",
...
seq(
    ...
    "ui",
    "wf",
    "start_workflow(invoice_json)  or  track_task(task_id)",
    color="#2f9e44",
)
seq("s02", "wf", "act", "execute_activity(start_task, invoice_json)", color="#f08c00")
seq("s03", "act", "mcp", "tools/call process_invoice  [JSON-RPC id:1]", color="#228be6")
seq("s04", "mcp", "stemp", "start InvoiceWorkflow", color="#e67700")
seq(
    "s05",
    "mcp",
    "act",
    "← taskId, status:working  [id:1]",
    color="#228be6",
    dashed=True,
)
seq("s06", "act", "wf", "← returns taskId", color="#f08c00", dashed=True)

# ── Phase 2: Polling loop ─────────────────────────────────────
phase_label("ph2", "Phase 2: Polling loop")
poll_sy = _y[0] - 8
seq("s07", "wf", "act", "execute_activity(poll_task_status, taskId)", color="#f08c00")
seq("s08", "act", "mcp", "tasks/get(taskId)  [JSON-RPC id:2]", color="#228be6")
seq("s09", "mcp", "act", "← status:working  [id:2]", color="#228be6", dashed=True)
seq(
    "s10",
    "act",
    "wf",
    "← 'working'  →  workflow.sleep(2s)",
    color="#f08c00",
    dashed=True,
)
seq(
    ...
    "wf",
    "act",
    "execute_activity(poll_task_status, taskId)  [after sleep]",
    color="#f08c00",
)
seq("s12", "act", "mcp", "tasks/get(taskId)  [JSON-RPC id:3]", color="#228be6")
seq(
    "s13", "mcp", "act", "← status:input_required  [id:3]", color="#e03131", dashed=True
)
seq("s14", "act", "wf", "← 'input_required'", color="#f08c00", dashed=True)
poll_ey = _y[0]
...
phase_label(
    "ph3",
    "Phase 3: Elicitation  (activity holds connection; human decision decoupled)",
    color="#9c36b5",
)
elix_sy = _y[0] - 8
...
seq("s25", "act", "wf", "← handle_elicitation returns", color="#f08c00", dashed=True)
...
poll2_sy = _y[0] - 8
seq("s26", "wf", "act", "execute_activity(poll_task_status, taskId)", color="#f08c00")
seq("s27", "act", "mcp", "tasks/get(taskId)  [JSON-RPC id:5]", color="#228be6")
seq("s28", "mcp", "act", "← status:completed  [id:5]", color="#2f9e44", dashed=True)
seq("s29", "act", "wf", "← 'completed'", color="#f08c00", dashed=True)
poll2_ey = _y[0]
bracket("poll2_b", "polling loop", poll2_sy, poll2_ey)

# ── Phase 5: Fetch final result ───────────────────────────────
phase_label("ph5", "Phase 5: Fetch final result")
seq("s30", "wf", "act", "execute_activity(get_task_result, taskId)", color="#f08c00")
seq(
    "s31",
    "act",
    "mcp",
    "tasks/result(taskId)  [JSON-RPC id:6]",
    color="#228be6",
...
seq(
    ...
    "mcp",
    "act",
    "← CallToolResult('PAID')  [id:6]",
    color="#2f9e44",
    dashed=True,
)
seq("s33", "act", "wf", "← 'Invoice PAID'", color="#f08c00", dashed=True)
seq("s34", "wf", "wf", "")  # placeholder for workflow.return
E.pop()  # remove that placeholder arrow
_y[0] -= 50

seq("s35", "ui", "wf", "query result  (or await workflow handle)", color="#2f9e44")
seq("s36", "wf", "ui", "← 'Invoice PAID'", color="#2f9e44", dashed=True)

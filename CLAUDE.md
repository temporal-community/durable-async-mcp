...
**Elicitation + sequential reader**: The MCP session's `_receive_loop` dispatches `elicitation/create` inline — the reader cannot process other messages while the handler runs. With N concurrent tasks all calling `tasks/result`, a handler that polls for minutes would starve `start_task` and other activities. Fix: the handler checks once (~20ms) and raises if no decision, releasing the reader. Temporal retries `handle_elicitation` (capped at 10s backoff) — brief turns on the reader rather than holding it. This is a protocol design gap: Elicitation predates Tasks and assumes one interaction at a time.

**`ui.py`** — Interactive CLI. Connects to Temporal only; no `fastmcp.Client`. Before each prompt, polls all running `TaskTrackerWorkflow` instances for pending elicitations and handles one at a time. Key operations: `start_workflow(TaskTrackerWorkflow.run, ...)`, `handle.query(get_elicitation_details)`, `handle.signal(user_decision, decision)`.

### Legacy MCP CLI Client (`async_mcp/mcp_client/`)
Older interactive CLI client that uses an LLM (OpenAI Responses API) to drive MCP tool calls. Requires `OPENAI_API_KEY`.
